  ...
  (let* ((file-list (cadr (file-glob (string-append folder-path "/*.svg") 1))))
    (while (not (null? file-list))
      (let* ((file (car file-list))
             ...
             (png-file (string-append file-name-no-ext ".png"))) ; Corresponding PNG file
        ; Check if PNG file already exists
        (if (file-exists? png-file)
            (begin
              (display (string-append "Skipping existing file: " png-file))
              (newline))
            (begin
              ; Convert SVG to PNG
              (let* ((image (car (gimp-file-load RUN-NONINTERACTIVE file file)))
                ...
                (file-png-save RUN-NONINTERACTIVE image drawable
                               png-file ; Output file name without .svg extension
                               png-file ; Output file name without .svg extension
